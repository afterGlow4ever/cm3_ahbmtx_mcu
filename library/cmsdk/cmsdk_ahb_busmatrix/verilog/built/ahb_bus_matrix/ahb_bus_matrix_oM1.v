...
// -----------------------------------------------------------------------------
    ...
    input         held_tran_op1;  // Port 1 held_tran_op signal

    input         HREADYOUTM; // HREADY feedback

    output        active_op1;    // Port 1 active_op signal

    // Slave addr_opess/Control Signals
    output        HSELM;      // Slave select line
    output [31:0] HADDRM;     // addr_opess
    output [31:0] HAUSERM;    // User addr_opess bus
...
// -----------------------------------------------------------------------------
    ...
    wire        held_tran_op1;  // Port 1 held_tran_op signal
    wire        active_op1;    // Port 1 active_op signal

    // Slave addr_opess/Control Signals
    wire        HSELM;      // Slave select line
    wire [31:0] HADDRM;     // addr_opess
    wire [31:0] HAUSERM;    // User addr_opess bus
    ...
    wire [31:0] HWUSERM;    // User data bus
...
// -----------------------------------------------------------------------------
  ...
  assign active_op1 = (addr_in_port == 2'b01) & !no_port;

  // addr_opess/control output decode
  assign i_hselm     = (addr_in_port == 2'b01) & sel_op1 & !no_port;
  assign HADDRM      = ( (addr_in_port == 2'b01) & !no_port ) ? addr_op1 : {32{1'b0}};
  assign HAUSERM     = ( (addr_in_port == 2'b01) & !no_port ) ? auser_op1 : {32{1'b0}};
  assign i_htransm   = ( (addr_in_port == 2'b01) & !no_port ) ? trans_op1 : 2'b00;
  assign HWRITEM     = (addr_in_port == 2'b01) & write_op1 & !no_port;
  assign HSIZEM      = ( (addr_in_port == 2'b01) & !no_port ) ? size_op1 : 3'b000;
  assign i_hburstm    = ( (addr_in_port == 2'b01) & !no_port ) ? burst_op1 : 3'b000;
  assign HPROTM      = ( (addr_in_port == 2'b01) & !no_port ) ? prot_op1  : {4{1'b0}};
  assign HMASTERM    = ( (addr_in_port == 2'b01) & !no_port ) ? master_op1 : 4'b0000;
  assign i_hmastlockm = (addr_in_port == 2'b01) & mastlock_op1 & !no_port;
  ...
  assign HWDATAM = ( data_in_port == 2'b01 ) ? wdata_op1 : {32{1'b0}};

  // HWUSERM output decode
  assign HWUSERM = ( data_in_port == 2'b01 ) ? wuser_op1 : {32{1'b0}};
