...
11	学习以太网：
(1)	学习以太网SMA的硬件RTL实现。
(2)	对以太网SMA进行功能测试和spyglass检查，修改其中的错误和问题。
(3)	学习以太网SMA的软件驱动编写。
(4)	掌握如何使用以太网SMA并实现特定的功能。
12	学习VCS与Verdi联合仿真环境：
(1)	掌握如何使用VCS进行仿真并使用Verdi记录波形。
13	学习gcc-arm-none-eabi编译firmware环境：
(1)	掌握如何使用gcc-arm-none-eabi编一个cpu可执行的程序。
14	学习Quartus和Vivado的fpga综合编译环境：
...
2	工程项目目录下User\Verilog，这里是MCU的基本集成结构，另外包含Uart、GPIO、基本定时器、以太网SMA外设。
3	工程项目目录下Library\drivers和Library\cmsis，这里是MCU的基本软件环境和Uart、GPIO、基本定时器、以太网SMA的RTL的配套驱动程序。
